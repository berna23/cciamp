        ...
        <div className="mb-12 border-b border-gray-100 pb-8 flex flex-col md:flex-row justify-between items-end gap-6">
           <div>
              <button 
                ...
                className="text-xs font-bold uppercase tracking-widest text-gray-400 hover:text-slate-900 mb-4 flex items-center gap-2 transition-colors"
              >
           ...
           {step < 5 && (
             <div className="text-xs font-bold uppercase tracking-widest text-gray-400">
               Passo 0{step} <span className="text-gray-200 mx-2">/</span> 04
             </div>
           )}
        </div>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="step1" variants={fade} initial="initial" animate="animate" exit="exit" className="grid md:grid-cols-2 gap-4">
               {categories.map((cat) => (
                 <button 
                   ...
                   className="group p-8 border border-gray-200 hover:border-slate-900 hover:bg-slate-50 transition-all text-left"
                 >
                    <h3 className="font-serif text-xl text-slate-900 mb-1 group-hover:text-accent transition-colors">{cat}</h3>
                    <p className="text-xs text-gray-400 uppercase tracking-wider">Selecionar</p>
                 </button>
               ))}
            </motion.div>
          )}
             ...
             <motion.div key="step2" variants={fade} initial="initial" animate="animate" exit="exit" className="space-y-4">
                {filteredProfessionals.map((prof) => (
                   <div 
                    ...
                    className="group flex flex-col sm:flex-row items-center gap-8 p-8 border border-gray-200 hover:border-slate-900 hover:bg-slate-50 transition-all cursor-pointer"
                   >
                      ...
                      <div className="flex-1 text-center sm:text-left">
                         <h3 className="font-serif text-xl text-slate-900 mb-1">{prof.name}</h3>
                         <p className="text-xs font-bold uppercase tracking-widest text-accent mb-2">{prof.role}</p>
                         <p className="text-sm text-gray-500 font-light line-clamp-2 max-w-md">{prof.shortBio}</p>
                      </div>
                      <div className="w-8 h-8 flex items-center justify-center border border-gray-200 group-hover:border-slate-900 transition-colors">
          ...
          {step === 3 && (
            <motion.div key="step3" variants={fade} initial="initial" animate="animate" exit="exit" className="grid lg:grid-cols-12 gap-12">
               ...
               {/* Calendar */}
               <div className="lg:col-span-7 border border-gray-200 p-8">
                  <div className="flex justify-between items-center mb-8">
                     <span className="font-serif text-lg">{viewDate.toLocaleDateString('pt-PT', { month: 'long', year: 'numeric' })}</span>
                     <div className="flex gap-2">
                        <button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() - 1, 1))} className="p-2 border border-gray-200 hover:bg-gray-100"><ChevronLeft size={16}/></button>
                        <button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() + 1, 1))} className="p-2 border border-gray-200 hover:bg-gray-100"><ChevronRight size={16}/></button>
                     </div>
                  </div>
                  
                  <div className="grid grid-cols-7 gap-1 text-center mb-2">
                    {['D','S','T','Q','Q','S','S'].map(d => <span key={d} className="text-[10px] font-bold text-gray-400 uppercase">{d}</span>)}
                  </div>
                     ...
                     {generateCalendarGrid().map((date, i) => (
                        <button 
                          ...
                          className={`h-10 text-sm font-medium transition-colors ${
                             !date ? 'invisible' :
                             isSameDay(bookingState.date, date) ? 'bg-slate-900 text-white' :
                             isPast(date) || date.getDay() === 0 ? 'text-gray-300 cursor-not-allowed' :
               ...
               <div className="lg:col-span-5 flex flex-col">
                  <div className="border border-gray-200 p-8 h-full">
                     ...
                     {bookingState.date ? (
                              ...
                              <button 
                                ...
                                className={`py-2 text-xs font-bold border transition-colors ${
                                  bookingState.time === time 
                                    ...
                                    : 'border-gray-200 hover:border-slate-900 text-gray-600'
                                }`}
                              >
                                {time}
                              </button>
                        ...
                        <p className="text-sm text-gray-400 font-light">Selecione uma data primeiro.</p>
                     )}
                  </div>
          ...
          {step === 4 && (
             <motion.div key="step4" variants={fade} initial="initial" animate="animate" exit="exit">
                <div className="border border-gray-200 p-8 md:p-12">
                   <h3 className="font-serif text-2xl text-slate-900 mb-8">Dados Pessoais</h3>
                   <form onSubmit={submitBooking} className="space-y-6">
                      <div className="grid md:grid-cols-2 gap-6">
                         ...
                         <div className="space-y-2">
                            <label className="text-[10px] font-bold uppercase tracking-widest text-gray-500">Telefone</label>
                      ...
                      <div className="space-y-2">
                         <label className="text-[10px] font-bold uppercase tracking-widest text-gray-500">Notas (Opcional)</label>
                      ...
                      <div className="bg-gray-50 p-6 border border-gray-100 flex gap-4 items-start">
          ...
          {step === 5 && (
             <motion.div key="step5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="border border-green-100 bg-green-50/30 p-12 text-center">
                ...
                <h2 className="font-serif text-3xl text-slate-900 mb-4">Pedido Registado</h2>
                <p className="text-gray-600 font-light leading-relaxed max-w-lg mx-auto mb-8">
